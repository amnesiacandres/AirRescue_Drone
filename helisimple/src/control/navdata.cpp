            ...
            if( navdata_option_ptr->size == 0 )
            ...
            else
                {
                    switch( navdata_option_ptr->tag )
                        ...
                        default:
                            if (navdbgprint) INFO ("Tag %d is not a valid navdata option tag\n", (int) navdata_option_ptr->tag);
                            navdata_option_ptr = NULL;
                            break;
                        }
                }
...
static void navdata_write (int8_t *buffer, int32_t len)
{
	...
	navdata_udp_addr.sin_port        = htons(MY_PORT);
...
static void* navdata_loop(void *arg)
{
	...
	while ( nav_thread_alive ) {
		...
		if (navdbgprint) fprintf(stdout,"X-%i-%x-%i\n",dbgseq++,(int)navdata,size);
		if( navdata->header == NAVDATA_HEADER)
		{
			mykonos_state = navdata->mykonos_state;

			if (navdbgprint) fprintf(stdout,"0-%i-%x\n",dbgseq++,(int)navdata);	
			if( get_mask_from_state(navdata->mykonos_state, MYKONOS_COM_WATCHDOG_MASK) ) 
			{ 
				INFO ("[NAVDATA] Detect com watchdog %x\n",navdata->mykonos_state);
				sequence = NAVDATA_SEQUENCE_DEFAULT-1; 
				usleep(100000);
			...
			if( navdata->sequence > sequence) 
			{
				...
				if ( get_mask_from_state( mykonos_state, MYKONOS_NAVDATA_DEMO_MASK ))
				{
					...
					if( cks == navdata_cks )
					{
						...
						//helidata.y = (double)navdata_unpacked.navdata_demo.drone_camera_trans.y;
						//helidata.z = (double)navdata_unpacked.navdata_demo.drone_camera_trans.z;
					}
				}
				if (navdbgprint) fprintf(stdout,"e%i-%x\n",dbgseq++,(int)navdata);	
			} 
			else 
			{ 
